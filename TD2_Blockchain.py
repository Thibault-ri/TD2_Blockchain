...
def creation_seed(liste_bip_39):
    ...
    entropy_hash=hashlib.sha256(str(bin_entropy).encode()).hexdigest()
...
def verif_dic(mot,liste_bip_39):
    indice=-1
    for i in range(len(liste_bip_39)):
        if(mot==liste_bip_39[i]):
            indice=i
            break
    return(indice)
...
def entree_seed():
    ...
    while(bon_mot!=True):
        ...
        l_mots=[]
        for i in range(12):
        	l_mots.append(input("Entrez le mot n°"+str(i+1)+":"))
        for j in range(12):
            if(verif_dic(l_mots[j],liste_bip_39)==-1):
                print("La seed n'est pas valide veuillez l'entrer a nouveau:")
                bon_mot=False
                break
    for i in range(12):
